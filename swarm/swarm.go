...
import (
	...
	httpapi "github.com/yooba-team/yooba/swarm/api/http"
	"github.com/yooba-team/yooba/swarm/fuse"
	"github.com/yooba-team/yooba/swarm/log"
	"github.com/yooba-team/yooba/swarm/network"
	"github.com/yooba-team/yooba/swarm/network/stream"
	"github.com/yooba-team/yooba/swarm/pss"
	"github.com/yooba-team/yooba/swarm/state"
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = yooclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	yooclient := yooclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, yooclient)
	if err != nil {
		return nil, err
	}
