...
type downloadTester struct {
	...
	stateDb yoobadb.Database // Database used by the tester for syncing from peers
	peerDb  yoobadb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	testdb, _ := yoobadb.NewMemDatabase()
	...
	tester.stateDb, _ = yoobadb.NewMemDatabase()
...
func (dl *downloadTester) makeChain(n int, seed byte, parent *types.Block, parentReceipts types.Receipts, heavy bool) ([]common.Hash, map[common.Hash]*types.Header, map[common.Hash]*types.Block, map[common.Hash]types.Receipts) {
	...
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent, dpos.NewFaker(), dl.peerDb, n, func(i int, block *core.BlockGen) {
		...
		if parent == dl.genesis && i%3 == 0 {
			...
			tx, err := types.SignTx(types.NewTransaction(block.TxNonce(testAddress), common.Address{seed}, big.NewInt(1000), params.TxGas, nil, types.TxTypeTransfer,nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
