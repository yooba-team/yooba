...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	case 1:
		...
		trie, _ := trie.New(root, trie.NewDatabase(yoobadb.NewTable(db, light.ChtTablePrefix)))
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, trie.NewDatabase(yoobadb.NewTable(db, light.BloomTrieTablePrefix)))
...
func TestTransactionStatusLes2(t *testing.T) {
	...
	tx0, _ := types.SignTx(types.NewTransaction(0, acc1Addr, big.NewInt(10000), params.TxGas, nil, types.TxTypeTransfer,nil), signer, testBankKey)
	test(tx0, true, txStatus{Status: core.TxStatusUnknown, Error: core.ErrUnderpriced.Error()})

	tx1, _ := types.SignTx(types.NewTransaction(0, acc1Addr, big.NewInt(10000), params.TxGas, big.NewInt(100000000000), types.TxTypeTransfer,nil), signer, testBankKey)
	...
	tx2, _ := types.SignTx(types.NewTransaction(1, acc1Addr, big.NewInt(10000), params.TxGas, big.NewInt(100000000000), types.TxTypeTransfer,nil), signer, testBankKey)
	tx3, _ := types.SignTx(types.NewTransaction(2, acc1Addr, big.NewInt(10000), params.TxGas, big.NewInt(100000000000), types.TxTypeTransfer,nil), signer, testBankKey)
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), dpos.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), dpos.NewFaker(), db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
