...
// NewProtocolManager returns a new Yooba sub protocol manager. The Yooba sub protocol manages peers capable
// with the Yooba network.
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb yoobadb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Yooba protocol")
	...
	log.Info("Light Yooba protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Yooba peer connected", "name", p.Name())
	...
	if err := p.Handshake( hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Yooba handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Yooba peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Yooba message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Yooba message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case AnnounceMsg:
		...
		p.Log().Trace("Announce message content", "number", req.Number, "hash", req.Hash, "td", "reorg", req.ReorgDepth)
		if pm.fetcher != nil {
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(yoobadb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				if root, prefix = pm.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(yoobadb.NewTable(pm.chainDb, prefix)))
				}
			}
...
// NodeInfo represents a short summary of the Yooba sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Yooba network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
