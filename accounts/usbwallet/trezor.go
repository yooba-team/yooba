...
import (
	...
	"github.com/yooba-team/yooba/accounts/usbwallet/internal/trezor"
...
// or whether the Yooba app was not started on it.
...
// Yooba address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.YoobaAddress)
	if _, err := w.trezorExchange(&trezor.YoobaGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.YoobaTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.YoobaTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
