...
// faucet is a Yoo faucet backed by a light client.
package main
...
var (
	...
	ethPortFlag = flag.Int("yooport", 31318, "Listener port for the devp2p connection")
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Yoos to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Yooba and the faucet")
)

var (
	yoobaYoo = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Yoos", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Yooba address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Yooba light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Yooba protocol stack
	client *yooclient.Client   // Client connection to the Yooba chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Yooba light client protocol
...
// close terminates the Yooba connection and tears down the faucet.
...
// apiHandler handles requests for Yoo grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, yoobaYoo),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Yooba address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/yooba-team/yooba/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), yoobaYoo)
			amount = new(big.Int).Mul(amount, new(big.Int).Exp(big.NewInt(5), big.NewInt(int64(msg.Tier)), nil))
			amount = new(big.Int).Div(amount, new(big.Int).Exp(big.NewInt(2), big.NewInt(int64(msg.Tier)), nil))

			tx := types.NewTransaction(f.nonce+uint64(len(f.reqs)), address, amount, 21000, f.price, types.TxTypeTransfer,nil)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, yoobaYoo)
...
// the username, avatar URL and Yooba address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Yooba addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Yooba address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Yooba address and profile URL.
...
// returning the username, avatar URL and Yooba address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Yooba address,
