...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the Yooba API provider to use to settle SWAP payments",
	...
	EnsAPIFlag = cli.StringFlag{
		Name:   "ens-api",
		Usage:  "URL of the Yooba API provider to use for ENS record lookups",
...
// This init function sets defaults so cmd/swarm can run alongside yooba.
func init() {
...
var app = utils.NewApp(gitCommit, "Yooba Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.yooba/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.yooba/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.yooba/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.yooba/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//yooba only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for yooba and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the yooba node
	...
	//register BZZ as node.Service in the Yooba node
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := yooclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if bzzconfig.EnsApi != "" {
			...
			ensClient = yooclient.NewClient(client)
	...
	//register within the Yooba node
