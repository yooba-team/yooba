...
// Header represents a block header in the Yooba blockchain.
type Header struct {
	header *types.Header
}
...
// Block represents an entire block in the Yooba blockchain.
type Block struct {
	block *types.Block
}
...
// Transaction represents a single Yooba transaction.
type Transaction struct {
	tx *types.Transaction
}

// NewTransaction creates a new transaction with the given properties.
func NewTransaction(nonce int64, to *Address, amount *BigInt, gasLimit int64, gasPrice *BigInt, txType uint,data []byte) *Transaction {
	return &Transaction{types.NewTransaction(uint64(nonce), to.address, amount.bigint, uint64(gasLimit), gasPrice.bigint, txType,common.CopyBytes(data))}
}
