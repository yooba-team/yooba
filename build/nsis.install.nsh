Name "yooba.${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install yooba binary
Section "yooba" YOOBA_IDX
  SetOutPath $INSTDIR
  file {{.yooba}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\yooba. exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\yooba. exe" "attach" "" ""
  ...
  # Firewall - add rules
  SimpleFC::AdvAddRule "yooba incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\yooba. exe" "" "" "Ethereum" 30303 "" "" ""
  SimpleFC::AdvAddRule "yooba outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\yooba. exe" "" "" "Ethereum" "" 30303 "" ""
  SimpleFC::AdvAddRule "yooba UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\yooba. exe" "" "" "Ethereum" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\yooba. ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\yooba. ipc"
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${YOOBA_IDX}
    SectionGetSize ${YOOBA_IDX} $0
