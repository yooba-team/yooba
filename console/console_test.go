...
func newTester(t *testing.T, confOverride func(*yoo.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Yooba service within
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return yoo.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Yooba protocol: %v", err)
	}
